...
import { X, Pencil, Save, Trash2, CheckCircle } from "lucide-react"; // Import thêm CheckCircle
...
const AddProductModal = ({
  ...
  // 3. STATE THÔNG BÁO THÀNH CÔNG
  const [showSuccess, setShowSuccess] = useState(false);

  // --- HÀM XỬ LÝ LƯU (CÓ DELAY & THÔNG BÁO) ---
  const handleSaveAction = () => {
    // 1. Hiện thông báo
    setShowSuccess(true);

    // 2. Đợi 1 giây rồi mới lưu thật
    setTimeout(() => {
      onSave(data);        // Lưu dữ liệu
      setIsEditing(false);  // Tắt chế độ sửa
      setShowSuccess(false); // Tắt thông báo
    }, 1000);
  };
  ...
  const stockInfo = [
    ...
    { label: "SKU", value: data.sku, key: "sku" },
  ];

  const additionalInfo = [
    { label: "Original Price", value: data.originalPrice, key: "originalPrice" },
  ];
